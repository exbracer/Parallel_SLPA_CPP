...
// Construnction function for MPI version
SLPA::SLPA(string inputFileName,vector<double> THRS,int maxRun,int maxT,string outputDir,bool isUseLargestComp,int numThreads, int proc_id, int num_procs) {
	//inputFileName: the full path
	//netName: short filename(non-suf)
	...
	this->start_index = new int[num_procs];
	this->end_index = new int[num_procs];
...
void SLPA::mpi_start(){
	...
	//     MPI split the node
	//---------------------------
	cout << "Num of Node_IDs = " << net->Node_IDs.size() << endl;
	decomposeTasks(net->N, num_procs, start_index, end_index);
	cout << "rank " << proc_id << ", start_index = " << start_index[proc_id] << ", end_index = " << end_index[proc_id] << endl;
	//---------------------------
	//  	game
	//---------------------------
	for(int run=1;run<=maxRun;run++){
	...
	cout << "rank " << proc_id << " is done!" << endl;
} // end of mpi_start()
