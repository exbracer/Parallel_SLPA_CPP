...
map<string,string> readinParameter(int argc,char* argv[]){
	...
	usage+= "  -P the number of procs(for multi-processer)\n";
	...
	for (int i=1; i<argc-1; i++)    { //ignore the first one
		...
		if(arg.compare("-t")==0 || arg.compare("-ov")==0 || arg.compare("-r")==0 || arg.compare("-run")==0 || arg.compare("-i")==0 || arg.compare("-d")==0  || arg.compare("-L")==0  || arg.compare("-M")==0 || arg.compare("-P")==0 ){
...
int main(int argc, char* argv[]) {
	...
	if(inputFileName.size()==0 || !isFileExist(inputFileName.c_str())){
		...
		SLPA slpa(inputFileName,THRS,maxRun,maxT,outputDir,isUseLargestComp,numThreads, rank, size);
		
		cout<<"Running Time for Proc " << rank << " is :" <<difftime(time(NULL),st)<< " seconds."<<endl;
	}
